...
namespace GSI_Internal.Controllers
{
    public class OurCompanyInfoController : Controller
    {
        ...
        {
            var exisiting = _ourCompanyInfoRepo.GetAll().FirstOrDefault();
            if (exisiting == null)
            {
        ...
        {
            var data = _ourCompanyInfoRepo.GetAll().FirstOrDefault();
            var map = _mapper.Map<OurCompanyInfo_VM>(data);
            return View(map);
        }
        [HttpPost]
        public IActionResult Edit(OurCompanyInfo_VM Obj)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                _ourCompanyInfoRepo.UpdateObj(newObj);
                return RedirectToAction("Index");
